#include <stdint.h>
#include "aes128e.h"

/* Multiplication by two in GF(2^8). Multiplication by three is xtime(a) ^ a */
#define xtime(a) ( ((a) & 0x80) ? (((a) << 1) ^ 0x1b) : ((a) << 1) )


// block size
#define Nb 4

// The number of rounds 128 bit.
#define Nr 10

// key length
# define Nk 4

/* The S-box table */
static const unsigned char sbox[256] = {  };

/* The round constant table (needed in KeyExpansion) */
static const unsigned char rcon[10] = {
		0x01, 0x02, 0x04, 0x08, 0x10,
	    0x20, 0x40, 0x80, 0x1b, 0x36 };

/* The expansion of the input key into the key schedule */
/*	http://csrc.nist.gov/publications/fips/fips197/fips-197.pdf */
void keyExpansion(unsigned char *key, unsigned char *roundKey,  int keyLength) {
	int i = 0, j;
	unsigned char temp[4], k;

	// Assigning same key for the first round.
	while (i < keyLength) {
		roundKey[i*4] = key[i*4];
		roundKey[i*4 + 1] = key[i*4 + 1];
		roundKey[i*4 + 2] = key[i*4 + 2];
		roundKey[i*4 + 3] = key[i*4 + 3];

		i++;
	}

	i = keyLength;

	while (i < (Nb * (Nr + 1))) {
		for (j = 0; j < 4; j++) {
			temp[j] = roundKey[(i - 1) * 4 + j];
		}

		if (i % keyLength == 0) {
			/* RotWord() -- cyclic permutation on the four byte input */
			{
				k = temp[0];
				temp[0] = temp[1];
				temp[1] = temp[2];
				temp[2] = temp[3];
				temp[3] = k;
			}

			/* Subword() -- applying s-box to the four-byte input word */
			{
				temp[0] = sbox[temp[0]];
				temp[1] = sbox[temp[1]];
				temp[2] = sbox[temp[2]];
				temp[3] = sbox[temp[3]];
			}
			temp[0] = temp[0] ^ rcon[i/keyLength-1];

		} else if (keyLength > 6 && i % keyLength == 4) {
			/* Subword() */
			{
				temp[0] = sbox[temp[0]];
				temp[1] = sbox[temp[1]];
				temp[2] = sbox[temp[2]];
				temp[3] = sbox[temp[3]];
			}
		}
		roundKey[i*4+0] = roundKey[(i-keyLength)*4+0] ^ temp[0];
		roundKey[i*4+1] = roundKey[(i-keyLength)*4+1] ^ temp[1];
		roundKey[i*4+2] = roundKey[(i-keyLength)*4+2] ^ temp[2];
		roundKey[i*4+3] = roundKey[(i-keyLength)*4+3] ^ temp[3];

		i++;
	}

}

/* XORs each column of state with a word from the key schedule ie.. roundKey and add it to state */
void addRoundKey(unsigned char state[4][4], unsigned char roundKey[176], int round) {
	int i, j;
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			state[j][i] ^= roundKey[round*Nb*4 + i*Nb + j];
		}
	}
}

/* Subbytes -- substituing each byte of state matrix using sbox table */
void subBytes(unsigned char state[4][4]) {
	int i, j;
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			state[i][j] = sbox[state[i][j]];

		}
	}
}

/* Shiftrows -- cyclically shifts the last three rows in the State over different offset
 * But not shifting the first row as offset being = 0.
 *  */
void shiftRows(unsigned char state[4][4]) {
	unsigned char temp;
	int i, j;
	for (i = 1; i<4; i++){
		int tempIndex = 0;
		temp = state[i][tempIndex];

		for (j = 0; j<4; j++){

			if (j == 3){
				state[i][tempIndex]  = temp;
				continue;
			}

			if (i == 2 && tempIndex == 2){
				state[i][i] = temp;
				tempIndex = i-1;
				continue;
			}

			int secIndex = (j+i)%4;
			if (secIndex == 0){
				if (i == 2){
					temp = state[i][tempIndex];
					secIndex = tempIndex+2;
				}
				if (i == 3){
					secIndex = i-j;
				}
			}

			state[i][tempIndex] = state[i][secIndex];
			tempIndex = secIndex;

		}

	}
}


/* transformation operation on the state column-by-column */
void mixColumns(unsigned char state[4][4]) {
	int i,j;
	unsigned char temp, tempXor, tempXtime;

	for (i = 0; i < 4; i++) {
		temp = state[0][i];
		tempXor = state[0][i] ^ state[1][i] ^ state[2][i] ^ state[3][i];

		for (j = 0; j< 4; j++){
			tempXtime = state[j][i] ^ state[(j+1)%4][i];
			if ((j+1)%4 == 0)tempXtime = state[j][i] ^ temp;
			tempXtime = xtime(tempXtime);
			state[j][i] ^= tempXtime ^ tempXor;
		}
	}
}

/* encrypt the plain text using the provided key */
/* http://csrc.nist.gov/publications/fips/fips197/fips-197.pdf */
/* Under the 16-byte key at k, encrypt the 16-byte plaintext at p and store it at c. */
void aes128e(unsigned char *c, const unsigned char *p, const unsigned char *k) {
	{
		int i, j = 0;

		unsigned char state[4][Nb];
		//unsigned char key[16];
		unsigned char roundKey[176];

		/* load the 16-byte plaintext at p into the state matrix
		 * load provided key to the
		 * */

		for (i = 0; i < Nk * 4; i++) {
			//key[i] = k[i];
			if (i < 4){
				for (j = 0; j < 4; j++) {
					state[j][i] = p[i*4 + j];
				}
			}
		}

		keyExpansion((unsigned char *)k, roundKey, Nk);

		addRoundKey(state, roundKey, 0);

		int round;

		for (round = 1; round < Nr; round++) {
			subBytes(state);
			shiftRows(state);
			mixColumns(state);
			addRoundKey(state, roundKey, round);
		}

		subBytes(state);
		shiftRows(state);
		addRoundKey(state, roundKey, Nr);

		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
				c[i*4+j] = state[j][i];
			}
		}

	}
}
